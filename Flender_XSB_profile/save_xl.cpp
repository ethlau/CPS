...
int main(int argc, char *argv[]){
  ...
  for(int i=0;i<nzbin;i++){
    ...
    for(int j=0;j<nmbin;j++){
      ...
      emission = calc_Flender_xray_emissivity_profile(cosm_model, zlist[i], Mlist[j], xlist); // ph/s/cm^3/arcsec^2
      double yp1 = 1.e31;
      double ypn = 1.e31;
	...
	/*
	for(int k=0;k<Nx;k++){
	  tab_yl_int[k] = x_l_integral(pow(10., tab_l_ls[k]), xout);
...
std::vector<double> calc_Flender_xray_emissivity_profile(cosmo cosm_model, float z, float Mvir, std::vector<float> x){
  ...
  */
  ...
  // distances in Mpc
  D_A = cosm_model.ang_diam(Redshift);
  D_L = cosm_model.lum_dist(Redshift);

  double angular_scale;
  
  angular_scale = 1.0/D_A; // radians/Mpc
  angular_scale *= 180.0/M_PI * 3600.0; // arcsecs/Mpc

  // redshift dependence in solid angle
  double fac = 4.0*M_PI*D_L*D_L*angular_scale*angular_scale;
  ...
  for(int xi=0;xi<x.size();xi++){
    ...
    if(r >= Rmax){emi = 0.0;}
        ...
        kT = pressure/ngas;
        emi = icm_mod.calc_xray_emissivity(ngas, kT, Redshift); // counts/s/cm^3
    ...
    emi = emi/fac/pow(1+Redshift, 3); // ergs/s/cm^3/arcsec^2
...
std::vector<double> calc_Shaw_xray_emissivity_profile(cosmo cosm_model, float z, float Mvir, std::vector<float> x){
  ...
  */
  ...
  for(int xi=0;xi<x.size();xi++){
    r = (double) x[xi]*R500;
    if(r >= Rmax){emi = 0.0;}
    else{emi = icm_mod.calc_xray_emissivity(r, R500, Redshift);} // ergs/s/cm^3
