  ...
  for(i=0;i<Nbin;i++){
	...
	*/
	...
	if(index < 0){
	  ...
	  xl = (tab_xl_int[index+1]-tab_xl_int[index])/(tab_l_ls[index+1]-tab_l_ls[index])
	    *(log10(l_ls)-tab_l_ls[index]) + tab_xl_int[index];
	  xl = xl * 4*M_PI*(r500/HubbleParam)/ells/ells; // count/s/cm^2/arcsec^2 
	}
	if(index < 0){
	  yl = 0;
	}else{
	  yl = (tab_yl_int[index+1]-tab_yl_int[index])/(tab_l_ls[index+1]-tab_l_ls[index])
	    *(log10(l_ls)-tab_l_ls[index]) + tab_yl_int[index];
